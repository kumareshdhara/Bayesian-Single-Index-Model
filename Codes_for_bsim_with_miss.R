...
wtd_quantile<-function(x, probs, quant){
  probs<-probs/sum(probs)
  
  newdata<-cbind(x, probs)
  sortnewdata<-newdata[order(x), ]
  
  cumsum_sortnewdata<-cumsum(sortnewdata[, 2])
  t<-min(which(cumsum_sortnewdata>quant))
  if(t>1){
    estimate<-sortnewdata[t-1,1]+(sortnewdata[t, 1]-sortnewdata[t-1, 1])*
      ((quant-cumsum_sortnewdata[t-1])/(sortnewdata[t,2]))
  }
  
  if(t==1){
    estimate<-sortnewdata[t, 1]
  }
...
bsim_miss_cov<-function(x, y, N, burn, grid.width=0.5)
{
  xobs<-x
  yobs<-y
  ...
  ### xobs_miss contains the missing data points
  xobs_miss<-xobs
  indices<-which(is.na(xobs[1,]))
  xobs<-xobs[,-indices]
  yobs<-yobs[-indices]
  
  n=n-sum(delta)   # Sample Size
  ...
  #Repeating everything to avoid an error. Hopefully I get rid of the error. 
  ...
  #generate from f
  sigma2<-(0.01^2) #Initial sigma^2
  #Sigma<-solve(C+diag(n)/sigma2)+diag(n)*0.005
  Sigma<-solve(Cinv+diag(n)/sigma2)#+diag(n)*0.005
  mu<-(Sigma%*%y)/sigma2
  ...
  a=2;
  b=0.01;
  #kappas<-as.matrix(seq(0.5,1.5,0.05))
  #probs<-matrix(0,length(kappas))
  
  N=N   ##Number of MCMC iterations
  ...
  means<-rowMeans((xobs_miss[, -indices]))
  covar<-var(t(xobs_miss[,-indices ]))
  sig12<-covar[1,2:3]
  sig21<-covar[2:3,1]
  sig11<-covar[1,1]
  sig22<-covar[2:3,2:3]
  
  codlsig<-sig11-sig12%*%solve(sig22)%*%sig21
  
  
  #xobs<-t(xobs)
  
  #mudata<-colMeans(xobs[!is.na(xobs[,1]),])
  codlmu1<-means[1]
  codlmu2<-means[2:3]
  codlmu<-rep(0,length(indices))
  for(i in 1:length(codlmu))
  {
    codlmu[i]<-codlmu1+sig12%*%solve(sig22)%*%(xobs_miss[2:3, indices[i]]-codlmu2)
  }
  
  
  weight<-matrix(0, N)
  logweight<-matrix(0, N)
  Q=50  ##Number of iterations for computing integral 
  weightcomp<-matrix(0, Q, N-burn+1)
  
  #weight calculation
  x_miss<-xobs_miss[,indices]
  y_miss<-(y_all[indices])
  ...
  for(k in burn:N)
  {
    ...
    for(j in 1:Q){
      ...
      x_aug<-x_miss
      x_aug[1,]<-rnorm(n=length(indices), mean=codlmu,sd=sqrt(codlsig))
      thetas<-storetheta[,k]
      
      alphas<-thetatoalpha(thetas)
      z<-as.matrix(t(x_aug))%*%alphas
      
      R1<-(matrix(1,length(z)))%*%t(z)
      R<-(abs(R1-t(R1)))             #need to square it to generate from GP 
      kappa<-storekappa[k]
      C<-exp(-kappa*R)
      
      sigmas2<-storesigma2[k]
      
      sig<-C+sigmas2*diag(length(indices))                                                                              
      weightcomp[j,k-burn+1]<-(-0.5*log(det(sig))-0.5*(t(y_miss)%*%solve(sig)%*%y_miss))
      
    }
  }
  
  
  weightcomp<-weightcomp-max(weightcomp)
  weightcomp<-exp(weightcomp)
  weight<-colSums(weightcomp)
  weightnew<- weight/sum(weight)
  esttheta<-storetheta[,burn:N]
  esttheta[1,]<-esttheta[1,]*weightnew
  esttheta[2,]<-esttheta[2,]*weightnew
  estimate<-rowSums(esttheta[])
  
  estimate<-rowSums(esttheta[])
  
  
  newdata1<-cbind(storetheta[1,burn:N], weightnew)
  newdata2<-cbind(storetheta[2,burn:N], weightnew)
  newdata1<-newdata1[order(newdata1[,1]),]
  newdata2<-newdata2[order(newdata2[,1]),]
  
  thetaci<-matrix(0, 2,3)
  thetaci[1,1]<-wtd_quantile(newdata1[,1], newdata1[,2], .025)
  thetaci[1,2]<-wtd_quantile(newdata1[,1], newdata1[,2], .5)
  thetaci[1,3]<-wtd_quantile(newdata1[,1], newdata1[,2], .975)
  
  thetaci[2,1]<-wtd_quantile(newdata2[,1], newdata2[,2], .025)
  thetaci[2,2]<-wtd_quantile(newdata2[,1], newdata2[,2], .5)
  thetaci[2,3]<-wtd_quantile(newdata2[,1], newdata2[,2], .975)
  
  
  allresults=c(theta_est, estimate, c(thetaci[1,], thetaci[2,]))
  allresults_csv<-c(iter, allresults)
  #  write.table(t(allresults_csv), 'Results_100_exp_sig_0_5_in_loop.csv', append=TRUE, col.names=FALSE, sep=',')
  return(list(theta_est=theta_est, thetaci=thetaci, storetheta=storetheta, weight=weightnew))
